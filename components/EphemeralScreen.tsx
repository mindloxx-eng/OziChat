...
const EphemeralScreen: React.FC<EphemeralScreenProps> = ({ contacts, onNavigateToProfile, navProps, onNavigateToVault }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Try S3 upload first, fallback to local data URL
      if (isAuthenticated()) {
        try {
          const res = await uploadMedia(file, 'status');
          if (res.success && res.data?.url) {
            contentUrl = res.data.url;
            console.log('🟢 Status media uploaded:', res.data.url);
          } else {
            contentUrl = await readFileAsDataUrl(file);
          }
        } catch (err) {
          console.warn('Status S3 upload failed, using local:', err);
          contentUrl = await readFileAsDataUrl(file);
        }
      } else {
        contentUrl = await readFileAsDataUrl(file);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#10101b] text-black dark:text-white transition-colors duration-300">
      ...
      <header className="p-4 pt-8 shrink-0 bg-white/80 dark:bg-[#10101b]/80 backdrop-blur-xl z-20 flex justify-between items-center border-b border-gray-100 dark:border-white/5">
