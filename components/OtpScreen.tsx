...
const OtpScreen: React.FC<OtpScreenProps> = ({ email, onOtpSuccess, onBackToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        // Tokens (if any) auto-saved by apiService.verifyOtp
        onOtpSuccess();
      } else {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-between p-8 bg-gradient-to-b from-[#0F172A] to-[#1E3A8A] text-white">
      <div className="w-full text-center">
        ...
        <p className="font-semibold text-[#3F9BFF] mt-1 tracking-wide">{email}</p>
      </div>
