...
const UserApp: React.FC<UserAppProps> = ({ contacts, settings, onUpdateContacts, onUpdateSettings }) => {
    ...
    if (appState === 'splash') {
      const timer = setTimeout(() => {
        ...
        if (hasOnboarded) {
            ...
            // New user: Show Ozi intro then onboarding
            setAppState('ozi-intro');
        }
      }, 1200); // Splash screen duration
  ...
  const handleOtpSuccess = () => {
    const setupStatus = localStorage.getItem('ozichat_setup_status');
    if (!setupStatus) {
  ...
  const handleNavigateToContactDetails = () => setAppState('contact-details');
  ...
  const renderContent = () => {
    ...
    switch (appState) {
      ...
      case 'login': return <LoginScreen onPhoneSubmit={handlePhoneSubmit} />;
      case 'otp': return <OtpScreen phoneNumber={phoneNumber} onOtpSuccess={handleOtpSuccess} onBackToLogin={handleBackToLogin} />;
      ...
      case 'chat':
        ...
        />;
      case 'profile': return <ProfileScreen onBack={handleBackToChat} onNavigateToVerification={() => setAppState('verification')} />;
      ...
      case 'create-group': return <CreateGroupScreen contacts={contacts} onBack={handleBackToContacts} onSaveGroup={handleSaveGroup} />;
      case 'media':
        if (!selectedChat) return <ContactsScreen chatList={chatList} onSelectChat={handleSelectChat} onNavigateToProfile={handleNavigateToProfile} onNavigateToAddContact={handleNavigateToAddContact} onNavigateToCreateGroup={handleNavigateToCreateGroup} onNavigateToSettings={handleNavigateToSettings} onNavigateToMarketplace={handleNavigateToMarketplace} onNavigateToAllContacts={handleNavigateToAllContacts} onNavigateToAI={handleNavigateToAIChat} navProps={{...navProps, activeScreen: 'chats'}} />;
